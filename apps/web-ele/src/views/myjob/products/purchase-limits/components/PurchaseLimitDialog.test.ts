...
import PurchaseLimitDialog from './PurchaseLimitDialog.vue';
...
describe('PurchaseLimitDialog', () => {
  ...
  it('disables submit until enum options finish loading', async () => {
    ...
    expect(apiMocks.addPurchaseLimitStrategyApi).not.toHaveBeenCalled();

    enumsDeferred.resolve({
      limit_types: [{ id: 1, title: '同一会员' }],
      period_types: [{ id: 2, title: '按区间(分钟)' }],
    });
    await flushPromises();
    await nextTick();
